...
mkdir -p ${datadir}/cnv 
cd ${scripts_root}
./filterAlnScoreAndQual.py -i $fin  -o $datadir/cnv/unambig_smurf_frags.sam -s 120 -q 1

samtools view -H $datadir/cnv/unambig_smurf_frags.sam  |\
...
samtools view -@16 -h -O BAM $datadir/cnv/unambig_smurf_frags.sam > $datadir/cnv/unambig_smurf_frags.bam
samtools reheader h.sam $datadir/cnv/unambig_smurf_frags.bam | samtools view -h -O SAM  - > $datadir/cnv/unambig_smurf_frags_chr.sam

./getBinCounts.py -v -i $datadir/cnv/unambig_smurf_frags_chr.sam  -c ../data/hg19.chrom.sizes -b ../data/bins_5k_hg19.bed -o $datadir/cnv/bin_counts.bed -s ../data/bin_stats.txt

cd $datadir/cnv/
Rscript $scripts_cnv $datadir/cnv/bin_counts.bed $sample ${scripts_data}/bins_5k_hg19_gc.txt ${scripts_data}/bins_5k_hg19_exclude.txt

rm $datadir/cnv/unambig_smurf_frags*
